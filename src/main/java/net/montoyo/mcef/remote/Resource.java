...
public class Resource {
	...
	/**
	 * Checks if the file exists. Then check if its checksum is valid. If the file
	 * couldn't be hashed, false will be returned.
	 * 
	 * @return true if (and only if) the file exists and the checksum matches the
	 *         {@link #sum} field.
	 */
	...
	public boolean download(IProgressListener ipl) {
		...
		return Util.download(MCEF.VERSION + '/' + platform + '/' + name + end, new File(ClientProxy.ROOT, name),
				shouldExtract, ipl);
	}
